...
#from .attentioner import AttentionAdapterBase, gpt2_attn, gptj_attn,AttentionerManagerBase,AttentionAdapter
...
class AttentionAdapter(AttentionAdapterBase):
    ...
    def _forward(self, attn_weights):
        ...
        if self.params is None:
            #print("位于modeling_qwen2参数不是None")
...
class Qwen2VLAttention(nn.Module):
    ...
    def __init__(self, config: Qwen2VLConfig, layer_idx: Optional[int] = None):
        ...
        self.attention_dapter = AttentionAdapter()
    ...
    def forward(
        ...
        #attn_weights_bs = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim)
        attn_weights_bs_ma = attn_weights + attention_mask

        if attention_mask is not None:  # no matter the length, we just slice it
            causal_mask = attention_mask[:, :, :, : key_states.shape[-2]]
            #attn_weights_bs_ma = attn_weights_bs + causal_mask
        ...
        #        )
            # # modify mask
            # upper_tri_mask = torch.triu(torch.ones(q_len, q_len)).cuda()
            # specified_value = -8888  # 你可以改成任意值，比如 0、1e9 等
            # attention_mask = attention_mask.masked_fill(upper_tri_mask == 1, specified_value)
        ...
        attn_weights = nn.functional.softmax(attn_weights_bs_ma, dim=-1, dtype=torch.float32).to(query_states.dtype)
        attn_weights = nn.functional.dropout(attn_weights, p=self.attention_dropout, training=self.training)
        attn_weights = self.attention_dapter(attn_weights)
        
        #attn_weights1 =  attn_weights + causal_mask
        ...
        #print("最后的输出attn_weights：",attn_weights)
        
        #print("最后的输出attn_weights_bs_ma：",attn_weights_bs_ma)
        #exit()
        #print("attn_weights的输出是：",attn_weights)
        #print("attn_weights_bs_ma",attn_weights_bs_ma)
        #print("最后的输出attn_output",attn_output)
        
        #exit()
        return attn_output, (attn_weights,attn_weights_bs_ma), past_key_value
...
class Qwen2VLFlashAttention2(Qwen2VLAttention):
    ...
    def forward(
        ...
        key_states = self.k_proj(hidden_states)
        value_states = self.v_proj(hidden_states)
        print("进入的ATTENTIONhanshu111111111111111111")
        query_states = query_states.view(bsz, q_len, -1, self.head_dim).transpose(1, 2)
        key_states = key_states.view(bsz, q_len, -1, self.head_dim).transpose(1, 2)
        value_states = value_states.view(bsz, q_len, -1, self.head_dim).transpose(1, 2)
...
class Qwen2VLDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        #print(f"attention_mask测试前的值是：d{attention_mask}")
        # Self Attention
        ...
        #print(f"attention_mask测试后的值是：d{attention_mask}")
        #print(f"attention_mask,{attention_mask}")
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        #print("测试是否进入了QvenVL model函数 111111111111111111111111111111111111111111111111111111111111111111111111111")
        if self.gradient_checkpointing and self.training:
            if use_cache:
                logger.warning_once(
        ...
        #print("测试是否进入了QvenVL model函数 99999999999999999999999999999999999999999999999")
