...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        else: #q,k,v : [b,624,4096]  heads : 32  head_dim:128  4096 = 32 * 128
        ...
        # query : [b,heads,seq_len,head_dim]  key : [b,heads,seq_len,head_dim]
        attn_weights_bs = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(self.head_dim)

        
        # attn_weights [b,heads,seq_len,seq_len]
        if attn_weights_bs.size() != (bsz, self.num_heads, q_len, kv_seq_len):
            raise ValueError(
                f"Attention weights should be of size {(bsz, self.num_heads, q_len, kv_seq_len)}, but is"
                f" {attn_weights_bs.size()}"
            )
        #print("现在是在llama的Attention中观察他的attention_mask：",attention_mask)
        #exit()
        if attention_mask is not None:
            ...
            attn_weights_bs_ma = attn_weights_bs + attention_mask
        ...
        # attn_output : [b,heads,seq_len,head_dim]  [1,32(heads),624(seq_len),128(head_dim)]
        ...
        # attn_output : [b,seq_len,hidden_dim]  [1,,624(seq_len),4096(head_dim)]
